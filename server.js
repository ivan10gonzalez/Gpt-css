...
app.get('/api/banners',master,(_q,s)=>s.json({banners:db.prepare('SELECT * FROM banners ORDER BY position,id DESC').all()}));
function validImage(f){if(!f)return false;const b=f.buffer;return (f.mimetype==='image/png'&&b.subarray(0,8).toString('hex')==='89504e470d0a1a0a')||(f.mimetype==='image/jpeg'&&b[0]===255&&b[1]===216&&b[2]===255)||(f.mimetype==='image/webp'&&b.toString('ascii',0,4)==='RIFF'&&b.toString('ascii',8,12)==='WEBP')||(f.mimetype==='image/gif'&&/^GIF8[79]a/.test(b.toString('ascii',0,6)));}
app.post('/api/banners',master,upload.single('image'),(q,s)=>{const {title='',subtitle='',category='all',position='0'}=q.body;if(!title.trim()||title.length>80||!['all','casino','live','sports'].includes(category)||!Number.isInteger(Number(position))||!validImage(q.file))return s.status(400).json({error:'Ingresá un título y una imagen PNG, JPG, WEBP o GIF válida (hasta 1,5 MB).'});const image=`data:${q.file.mimetype};base64,${q.file.buffer.toString('base64')}`;const r=db.prepare('INSERT INTO banners(title,subtitle,category,position,image) VALUES(?,?,?,?,?)').run(title.trim(),String(subtitle).slice(0,160),category,Number(position),image);s.json({ok:true,id:r.lastInsertRowid});});
app.patch('/api/banners/:id',master,(q,s)=>{if(typeof q.body.active!=='boolean'||!Number.isInteger(Number(q.body.position)))return s.status(400).json({error:'Estado u orden inválido.'});const r=db.prepare('UPDATE banners SET active=?,position=? WHERE id=?').run(q.body.active?1:0,Number(q.body.position),q.params.id);s.status(r.changes?200:404).json(r.changes?{ok:true}:{error:'Banner no encontrado.'});});
